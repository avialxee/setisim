...
from casatasks import tclean, split, listobs, plotants, imstat, clearcal, visstat, casalog, flagdata, flagmanager
...
def selfcal_model(calibrated_target, goodchans='0:500~600', ref_ant='C00'):
    ...
    gaincal(vis=sctarget['sc2'], caltable=sgtab['sc3'], 
            field='0', spw='0', refant=ref_ant, calmode='ap', solint='10s', combine='', 
            minsnr=3.0)
...
def cli():
    ...
    if pp == 111:
        ...
        print(metadata['scan_'+s['targets']]['0'])
    ...
    if timeinstant:
            ...
            tr = TimeDelta(timeresolution, format='sec', scale=str.lower(metadata['timeref']))
    ...
    if pp == 10:
        ...
        print(stat)
        print(stat['max'][0])
        # genpng(imagename,0,out=f"{out}", kind='jpg', norm_max=stat['max'][0])
